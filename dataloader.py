...
def get_imdb(batch_size, max_length):
    ...
    train, test = TabularDataset.splits(
               path="dataset/drug-data/", # the root directory where the data lies
               train='drugsTrain_raw_clean.csv', test="drugsTest_raw_clean.csv",
               format='csv',
...
if __name__ == "__main__":
    ...
    for i, b in enumerate(iter(train)):
        if i > 2:
            break
        print(i, num2words(vocab, b.review[0][0].numpy()))
        cnt[b.rating[0].item()] += 1
    print(cnt)
